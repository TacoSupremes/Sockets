...
public class ItemSoulGem extends ItemMod {
	...
	public void addInformation(ItemStack is, EntityPlayer player, List<String> l, boolean advanced) {
		...
		l.add(I18n.translateToLocal(LibMisc.MODID + "." + "contains") + " : "+(e.hasCustomName() ? e.getCustomNameTag() + " (" + n + ")" : n));
		
	if(Sockets.proxy.isShiftDown()){
		l.add(I18n.translateToLocal(LibMisc.MODID + "." + "health") + " : "+ e.getHealth() + " / " + e.getMaxHealth());
		
	}else
		l.add(I18n.translateToLocal(LibMisc.MODID + "." + "press") + " " + ChatFormatting.GREEN + I18n.translateToLocal(LibMisc.MODID + "." + "shift")+" "+ChatFormatting.GRAY+I18n.translateToLocal(LibMisc.MODID + "." + "moreInfo"));
	...
	public boolean onItemUse(ItemStack is, EntityPlayer player, World w, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ)
	    ...
	    public static Entity spawnCreature(World w, ItemStack is, double x, double y, double z)
	    {
	        ...
	        else
	        {
	            ...
	            for (int i = 0; i < 1; ++i)
	            {
	                entity = EntityList.createEntityByIDFromName(is.getTagCompound().getString("NAME"), w);
